  ...
  def __init__(self, vocab_file, max_size):
    ...
    with open(vocab_file, 'r') as vocab_f:
      for line in vocab_f:
        ...
        if len(pieces) != 2:
          # print vocab_file
          print 'Warning: fuck incorrectly formatted line in vocabulary file: %s\n %s' % (line,vocab_file)
          continue
        w = pieces[0]
...
def show_art_oovs(article, vocab):
  ...
  unk_token = vocab['data'].word2id(UNKNOWN_TOKEN)
  words = article.split(' ')
  words = [("__%s__" % w) if vocab['data'].word2id(w)==unk_token else w for w in words]
  out_str = ' '.join(words)
  return out_str
...
def show_abs_oovs(abstract, vocab, article_oovs):
  ...
  new_words = []
  for w in words:
    if vocab['data'].word2id(w) == unk_token: # w is oov
