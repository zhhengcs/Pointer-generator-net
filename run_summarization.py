...
from data import Vocab,example_generator
...
tf.app.flags.DEFINE_string('POS_vocab_path', '', 'Path expression to tf.Example POS_files.')
...
def run_training(model, batcher, sess_context_manager, sv, summary_writer):
	...
	with sess_context_manager as sess:
		...
		while True: # repeats until interrupted
			...
			loss = results['loss']
			tf.logging.info('batch %d,loss: %f',batch_cnt,loss) # print the loss to screen
			batch_cnt += 1
...
def main(unused_argv):
    ...
    # create a vocabulary
	vocab = dict()
	vocab['data'] = Vocab(FLAGS.vocab_path, FLAGS.vocab_size) 
	vocab['POS'] = Vocab(FLAGS.POS_vocab_path,0)
	...
	# D =  batcher.next_batch()
	# print(D.enc_batch[0])
	# input_gen = batcher.text_generator(example_generator(FLAGS.data_path, 1))
	
	# for item in input_gen:
	# 	print 'article',item[0]
	# 	print 'title',item[1]
	# 	print 'POS',item[2]
