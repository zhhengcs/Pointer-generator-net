...
class SummarizationModel(object):
  ...
  def __init__(self, hps, vocab):
    self._hps = hps
    self._vocab = vocab['data']
    self._POS_vocab = vocab['POS']
  ...
  def _add_placeholders(self):
    ...
    self._pos_enc_batch = tf.placeholder(tf.int32,[hps.batch_size,None],name='POS_batch')
  ...
  def _make_feed_dict(self, batch, just_enc=False):
    ...
    feed_dict[self._pos_enc_batch] = batch.pos_enc_batch
  ...
  def _add_seq2seq(self):
    ...
    psize = self._POS_vocab.size()

    with tf.variable_scope('seq2seq'):
      # Some initializers
      self.rand_unif_init = tf.random_uniform_initializer(-hps.rand_unif_init_mag, hps.rand_unif_init_mag, seed=123)
      self.trunc_norm_init = tf.truncated_normal_initializer(stddev=hps.trunc_norm_init_std)
      # word_matrix = prepare_feature.wordvec(train=False)
      # word_matrix.shape=(vsize,128)

      # self.wordvec_init = tf.constant_initializer(word_matrix)
      
      # Add embedding matrix (shared by the encoder and decoder inputs)
      with tf.variable_scope('embedding'):
        embedding = tf.get_variable('embedding', [vsize, hps.emb_dim], dtype=tf.float32, initializer=self.trunc_norm_init,trainable=True)
        POS_embedding = tf.get_variable('POS_embedding', [psize, hps.emb_dim], dtype=tf.float32, initializer=self.trunc_norm_init,trainable=True)

        if hps.mode=="train": self._add_emb_vis(embedding) # add to tensorboard
        emb_data_inputs = tf.nn.embedding_lookup(embedding, self._enc_batch) # tensor with shape (batch_size, max_enc_steps, emb_size)
        emb_pos_enc_inputs = tf.nn.embedding_lookup(POS_embedding,self._pos_enc_batch)#tensor with shape (batch_size, max_enc_steps, emb_size)
        
        emb_enc_inputs = tf.concat(axis =2,values = [emb_data_inputs,emb_pos_enc_inputs])
        # print('********************',emb_data_inputs.shape)
        # emb_enc_inputs = emb_data_inputs
        # emb_enc_inputs 
    ...
    if hps.mode == "decode":
      ...
      topk_probs, self._topk_ids = tf.nn.top_k(final_dists, FLAGS.max_enc_steps)
